...
const createHappyPlugin = (id, loaders) => {
    return new HappyPack({
        id,
        loaders,
        threadPool: happyThreadPool, // retrieving worker threads
        verbose: false // log status messages
    });
};
...
if (Array.isArray(projectConfig.styles)) {
    if (projectConfig.styles.includes('less')) {
...
const vueStyleLoaders = styleLanguageList.reduce((prev, cur) => {
    if (isDevelopment) {
        prev[cur.language] = ['vue-style', ...cur.loaders].map(
            v => `${v}-loader`
        );
    } else {
