...
const ProvideMap = plugins.find(v => v.constructor.name === 'ProvidePlugin')
    .definitions;
const ProvideGlobals = Object.keys(ProvideMap).reduce((prev, cur) => {
    prev[cur] = false;
    return prev;
}, {});
...
const NoRestrictedGlobals = ['location', 'navigator', 'screen'].map(v => ({
    name: v,
    message: `请使用显式全局变量 window.${v}`
}));

module.exports = {
    parser: 'babel-eslint',
    extends: ['eslint:recommended', 'airbnb', 'plugin:vue/base'],
    parserOptions: {
        ecmaVersion: 6
    },
    plugins: ['html', 'import'],
